...
void Utils::uinput_availability_test() {
	int fd_test = open("/dev/uinput", O_RDWR);

	if (fd_test == -1) {
		std::cerr << "ydotool: Error: unable to open /dev/uinput: " << strerror(errno) << ". You may not have sufficient permissions.\n\n";
		std::cerr << "See https://github.com/ReimuNotMoe/ydotool/issues/25#issuecomment-535842993 for more information.\n\n";
		abort();
	}
